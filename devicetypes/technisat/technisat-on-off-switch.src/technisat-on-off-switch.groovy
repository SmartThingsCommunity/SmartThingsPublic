...
metadata 
{
              ...
              executeCommandsLocally: false, genericHandler: "Z-Wave")
  {
    ...
    // costum commands
	command "reset"
	// zw:Ls2a type:1001 mfr:0299 prod:0002 model:1A90 ver:2.00 zwv:6.08 lib:03 cc:5E,55,6C,98,9F sec:25,32,59,5A,5B,70,71,72,73,7A,85,86
	fingerprint mfr: "0299", prod: "0002", model: "1A90", deviceJoinName: "TechniSat On/Off switch"
  }
...
// preferences -> used in settings menu
  preferences 
  {
	parameterMap.each 
    {
...
// report i'm alive for health check capability. called whenever installed or updated:
def initialize() 
{
...
// default Z-Wave report handler -> just log
def zwaveEvent(physicalgraph.zwave.Command cmd) {
	log.debug "${device.displayName}: Unhandled: $cmd"
	[:]
}

// on & off required by Switch and Light capability
def on() {
	encapSequence([
...
// implementet poll & ping... Who kowns what is used...?

// PING is used by Device-Watch in attempt to reach the Device -> call refresh
def ping() {
	log.debug "ping()"
	refresh()
}

// required by poll capability -> just init a refresh
def poll() {
	sendHubCommand(refresh())
}

// required by refresh capability, updates all values
def refresh() {
	log.debug "refresh()"
	encapSequence([
		zwave.switchBinaryV1.switchBinaryGet(),
...
private isConfigChanged(parameter)
{
  ...
  // first check the enableswitch ids it exists
  if(parameter.enableSwitch)
  {
    if(settings."$parameter.enableKey" != null)
    {
      ...
      {
      // set value to 0 if exists and diabled
        settingsValue = 0;
      }
    }
  }
  ...
  if (settingsValue != null) 
  {
    ...
    // check if the value differs the saved one
    ...
    // check the status of the parameter, maybe the sync faild
...
private syncConfig()
{
  ...
  parameterMap.each
  {
    ...
    if (isConfigChanged(it)) 
    {
      ...
      //commands << "delay 500" // delay of 500ms ??                                                                 
...
// init the currentConfig database at install
...
// convert parameter value for setting config via Configuration CC
...
//parametermap used for preferences (settings menu)
