...
metadata {
    ...
    tiles (scale: 2) {
        ...
        standardTile("empty", "device.empty", decoration: "flat", width: 2, height: 1) {
            state "default", label:''
        }
...
def installed() {
    sendHubCommand(new physicalgraph.device.HubAction(zwave.multiChannelV3.multiChannelEndPointGet().format()))
}
...
def zwaveEvent(physicalgraph.zwave.commands.sensormultilevelv5.SensorMultilevelReport cmd) { createEvent(descriptionText: "Unallocated Sensor Multilevel Report: $cmd.scaledSensorValue") }
...
void createChildDevices(def cc, def ep) {
    try {
        ...
        runIn(2, "associationSet", [overwrite: true])
    } catch (e) {
...
def associationSet() {
    ...
    def cmd = "${multiChannelAssociationCC}${setCmd}${groupingIdentifier}${marker}${nodeId}${ep}"
...
def parentCommand(def cmd) {
    if (cmd) sendHubCommand(new physicalgraph.device.HubAction(cmd))
}
...
def encap(def source, def destination, def cmd) {
    ...
    def result = "${multiChannel}${cmdEncap}${sourceEP}${destinationEP}${cmd}"
    
    sendHubCommand(new physicalgraph.device.HubAction(result))
}
