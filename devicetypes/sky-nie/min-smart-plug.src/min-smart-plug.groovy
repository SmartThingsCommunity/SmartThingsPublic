...
def installed() {
    logDebug "installed()..."
    if (isNewModel() && state.debugLoggingEnabled == null) {
...
private static getLedModeOptions1() {
    return [
        "0":"Off When On",
        "1":"On When On",
        "2":"Always Off",
...
private isNewModel() {
    if(device == null){
        log.error "isNewModel device = null"
        return true
    }else{
        log.error "isNewModel device.rawDescription = ${device.rawDescription}"
