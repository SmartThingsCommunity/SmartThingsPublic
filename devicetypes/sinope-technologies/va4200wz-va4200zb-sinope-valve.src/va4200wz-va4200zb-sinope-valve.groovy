...
private def convertVoltToPercent(value) {
	...
	if (value > anglesTable[anglesTable.size - 1]) { // if the value of the angle is greater than the maximum
		value = anglesTable[anglesTable.size - 1]; // use the maximum value instead
	}

	def index = 1;
    
    while ( value > anglesTable[index]) { index++ }
	...
	def roundedLevelValue = Math.round(levelWithFactor / 1000);
