 ...
 */
metadata {
	definition (name: "KeyWe Smart Locks", namespace: "KeyWe Smart Locks", author: "Youngmin Andy Kim", minHubCoreVersion: '000.017.0012', executeCommandsLocally: true) {
		capability "Actuator"
		capability "Lock"
		capability "Polling"
		capability "Refresh"
...
def installedCheck() {
	if (device.currentState("lock") && device.currentState("battery")) {unschedule("installedCheck")} 
	else {
		...
		if (!state.lastLockDetailsQuery || secondsPast(state.lastLockDetailsQuery, 2 * 60)) {
			def actions = updated()
			if (actions) {sendHubCommand(actions.toHubAction())}
		}
		scheduleInstalledCheck()
	}
}
...
def updated() {
	...
	try {
		...
		if (!device.currentState("lock") || !device.currentState("battery") || !state.configured) {
			...
			if (!state.configured) {cmds << doConfigure()}
			cmds << refresh()
			cmds << reloadAllCodes()
			if (!state.MSR) {cmds << zwave.manufacturerSpecificV1.manufacturerSpecificGet().format()}
			if (!state.fw) {cmds << zwave.versionV1.versionGet().format()}
			hubAction = response(delayBetween(cmds, 30*1000))
		}
	} catch (e) {log.warn "updated() threw $e"}
	hubAction
}
...
def parse(String description) {
	...
	if (description.startsWith("Err")) {
		if (state.sec) {result = createEvent(descriptionText:description, isStateChange:true, displayed:false)} 
		else {result = createEvent(descriptionText: "This lock failed to complete the network security key exchange. If you are unable to control it via SmartThings, you must remove it from your network and add it again.",eventType: "ALERT",name: "secureInclusion",value: "failed",displayed: true,)}
	} else {
...
def zwaveEvent(physicalgraph.zwave.commands.securityv1.SecurityCommandsSupportedReport cmd) {
	...
	if (cmd.commandClassControl) {state.secCon = cmd.commandClassControl.collect { String.format("%02X ", it) }.join()}
...
def zwaveEvent(physicalgraph.zwave.commands.alarmv2.AlarmReport cmd) {
	...
	def result = []
	if (cmd.zwaveAlarmType == 6) {result = handleAccessAlarmReport(cmd)} 
	else if (cmd.zwaveAlarmType == 10) {result = handleSecurityAlarmReport(cmd)}
...
// @param cmd: The AlarmReport command to be parsed
// @return The event(s) to be sent out
private def handleSecurityAlarmReport(cmd) {
	log.trace "[DTH] Executing 'handleSecurityAlarmReport' with cmd = $cmd"
	...
	if (cmd.zwaveAlarmEvent == 1) {
		map = [ name: "security", value: "door"]
		map.descriptionText = "Non-normal door opened"
		map.data = [ method: "door" ]
	} else if (cmd.zwaveAlarmEvent == 2) {
		map = [ name: "security", value: "fire"]
		map.descriptionText = "Fire detected"
		map.data = [ method: "fire" ]
	}
	
	if (map) {
		if (map.data) {map.data.lockName = deviceName} 
		else {map.data = [ lockName: deviceName ]}
...
private def handleAccessAlarmReport(cmd) {
	...
	def deviceName = device.displayName
	lockCodes = loadLockCodes()
	if (1 <= cmd.zwaveAlarmEvent && cmd.zwaveAlarmEvent < 10) {map = [ name: "lock", value: (cmd.zwaveAlarmEvent & 1) ? "locked" : "unlocked" ]}

	switch(cmd.zwaveAlarmEvent) {
		case 1: // Manually locked
			//map.descriptionText = "Locked manually"
			map.descriptionText = "Locked by inside button"
			map.data = [ method: "button" ]
			//map.data = [ method: (cmd.alarmLevel == 2) ? "keypad" : "manual" ]
			break
		case 2: // Manually unlocked
			//map.descriptionText = "Unlocked manually"
			map.descriptionText = "Unlocked by inside button"
			map.data = [ method: "button" ]
			//map.data = [ method: "manual" ]
			break
		case 3: // Locked by command
			map.descriptionText = "Locked by app"
			map.data = [ method: "command" ]
			break
		case 4: // Unlocked by command
			map.descriptionText = "Unlocked by app"
			map.data = [ method: "command" ]
			break
		...
		case 9: // Auto locked
			...
			break
		case 0xB:
			//map = [ name: "lock", value: "jammed", descriptionText: "Deadbolt jammed" ]
			map = [ name: "lock", value: "unknown", descriptionText: "Deadbolt jammed" ]
			break
		...
		case 0xF: // Duplicate Pin-code error
			...
			break
		case 0x18: // Unlocked by Key or Knob
			map = [ name: "lock", value: "unlocked", descriptionText: "Unlocked by key or knob" ]
			map.data = [ method: "Key or Knob" ]
			break
		case 0x19: // Locked by Key or Knob
			map = [ name: "lock", value: "locked", descriptionText: "Locked by key or knob" ]
			map.data = [ method: "Key or Knob" ]
			break
	}
...
def zwaveEvent(UserCodeReport cmd) {
	...
	if (userIdStatus == UserCodeReport.USER_ID_STATUS_OCCUPIED || (userIdStatus == UserCodeReport.USER_ID_STATUS_STATUS_NOT_AVAILABLE && cmd.user)) {
		...
		if (!lockCodes[codeID]) {result << codeSetEvent(lockCodes, codeID, codeName)} 
		else {map.displayed = false}
	...
	if (codeID.toInteger() == state.checkCode) {  // reloadAllCodes() was called, keep requesting the codes in order
		...
		//if (state.checkCode + 1 > state.codes) {			
	...
	if (codeID == state.pollCode) {
		if (state.pollCode + 1 > state.codes || state.pollCode >= 15) {
			...
			state["pollCode"] = null
		} else {state.pollCode = state.pollCode + 1}
	}
...
def zwaveEvent(UsersNumberReport cmd) {
	...
	if (state.checkCode) {
		if (state.checkCode <= cmd.supportedUsers) {result << response(requestCode(state.checkCode))} 
		else {
...
def zwaveEvent(physicalgraph.zwave.commands.associationv2.AssociationReport cmd) {
	...
	if (cmd.nodeId.any { it == zwaveHubNodeId }) {
		...
		if (cmd.groupingIdentifier == 2) {result << response(zwave.associationV1.associationRemove(groupingIdentifier:1, nodeId:zwaveHubNodeId))}
	} 
	else if (cmd.groupingIdentifier == 1) {result << response(secure(zwave.associationV1.associationSet(groupingIdentifier:1, nodeId:zwaveHubNodeId)))} 
	else if (cmd.groupingIdentifier == 2) {result << response(zwave.associationV1.associationSet(groupingIdentifier:2, nodeId:zwaveHubNodeId))}
	result
}
...
def zwaveEvent(physicalgraph.zwave.commands.timev1.TimeGet cmd) {
	...
	result << response(secure(zwave.timeV1.timeReport(hourLocalTime: now.get(Calendar.HOUR_OF_DAY),minuteLocalTime: now.get(Calendar.MINUTE),secondLocalTime: now.get(Calendar.SECOND))))
	result
}
...
def lockAndCheck(doorLockMode) {
	//secureSequence([zwave.doorLockV1.doorLockOperationSet(doorLockMode: doorLockMode)], 4200)
	secureSequence([zwave.doorLockV1.doorLockOperationSet(doorLockMode: doorLockMode),zwave.doorLockV1.doorLockOperationGet()], 6000)
}
...
def poll() {
	...
	if (state.assoc != zwaveHubNodeId && secondsPast(state.associationQuery, 19 * 60)) {
		...
		else if (!state.MSR) {cmds << zwave.manufacturerSpecificV1.manufacturerSpecificGet().format()} 
		else if (!state.fw) {cmds << zwave.versionV1.versionGet().format()} 
		...
		else if (!state.lastbatt || now() - state.lastbatt > 53*60*60*1000) {cmds << secure(zwave.batteryV1.batteryGet())}
	}
...
def reloadAllCodes() {
	...
	if (!state.codes) {cmds << secure(zwave.userCodeV1.usersNumberGet())}
	else {
		sendEvent(name: "maxCodes", value: state.codes, displayed: false)
		cmds << requestCode(state.checkCode)
	}
	if(cmds.size() > 1) {cmds = delayBetween(cmds, 4200)}
	cmds
}
...
def setCode(codeID, code, codeName = null) {
	...
	def strcode = code
	if (code instanceof String) {code = code.toList().findResults { if(it > ' ' && it != ',' && it != '-') it.toCharacter() as Short }} 
	else {strcode = code.collect{ it as Character }.join()}
...
def validateAttributes() {
	def cmds = []
	if(!device.currentValue("maxCodes")) { cmds << secure(zwave.userCodeV1.usersNumberGet()) }
...
def updateCodes(codeSettings) {
	...
	if (set_cmds) { return response(delayBetween(set_cmds, 2200)) }
	return null
}
...
def deleteCode(codeID) {
	...
	secureSequence([zwave.userCodeV1.userCodeSet(userIdentifier:codeID, userIdStatus:0),zwave.userCodeV1.userCodeGet(userIdentifier:codeID)], 4200)
}
...
private Boolean secondsPast(timestamp, seconds) {
	if (!(timestamp instanceof Number)) {
		if (timestamp instanceof Date) {timestamp = timestamp.time} 
		else if ((timestamp instanceof String) && timestamp.isNumber()) {timestamp = timestamp.toLong()} 
		else {return true}
	}
...
private String getCodeName(lockCodes, codeID) {
	if (isMasterCode(codeID)) { return "Master Code" }
	if (codeID.toString()=='240') { return "OTP key" }
	lockCodes[codeID.toString()] ?: "User ID : $codeID"	
}
...
private String getCodeNameFromState(lockCodes, codeID) {
	...
	if(nameFromLockCodes) {
		if(nameFromState) { return nameFromState }  //Updated from smart app
		else { return nameFromLockCodes } //Updated from lock
	} else if(nameFromState) { return nameFromState }  //Set from smart app
	//Set from lock
	return "Code $codeID"
}
...
private boolean isMasterCode(codeID) {
	if(codeID instanceof String) { codeID = codeID.toInteger() }
	(codeID == 0) ? true : false
}
...
private def getChangeType(lockCodes, codeID) {
	def changeType = "set"
	if (lockCodes[codeID.toString()]) { changeType = "changed" }
	changeType
}
...
private def getStatusForDescription(changeType) {
	if("set" == changeType) { return "Added" }
	else if("changed" == changeType) { return "Updated" }
...
def readCodeSlotId(physicalgraph.zwave.commands.alarmv2.AlarmReport cmd) {
	if(cmd.numberOfEventParameters == 1) { return cmd.eventParameter[0] }
	else if(cmd.numberOfEventParameters >= 3) { return cmd.eventParameter[2] }
