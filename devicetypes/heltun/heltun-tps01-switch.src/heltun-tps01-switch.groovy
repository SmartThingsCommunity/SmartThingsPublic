...
metadata {
	definition (name: "HELTUN TPS01 Switch", namespace: "HELTUN", author: "Sarkis Kabrailian", cstHandler: true, mcdSync: true ) {
		...
		fingerprint mfr: "0344", prod: "0004", model: "0007", deviceJoinName: "HELTUN Panel"
	}
	preferences {
		input (
			title: "HE-TPS01 | HELTUN Touch Panel Switch",
...
def zwaveEvent(physicalgraph.zwave.commands.switchbinaryv1.SwitchBinaryReport cmd, ep = null) {
	...
	if (ep in numberOfButtons..(2*numberOfButtons)) {
		def param = parameterMap().find( {it.paramNum == corRelParam } )
		if (param) {
			def paramName = param."name"
		}
		def paramState = state."$paramName"
		if (paramState) {
...
private parameterMap() {[
	[title: "Relay Output Mode", description: "This Parameter determine the type of loads connected to the device relay output. " + 
	"The output type can be NO – normal open (no contact/voltage switch the load OFF) or NC - normal close (output is contacted / there is a voltage to switch the load OFF)",
	 name: "Selected Relay Mode", options: [
				0: "NO - Normal Open", 
				1: "NC - Normal Close"
		], paramNum: 7, size: 1, default: "0", type: "enum"],

	[title: "Relay Load Power", description: "This parameter is used to specify the load power that is connected to the device output (Relay). " +
	"Using your connected device’s power consumption specification (see associated owner’s manual), set the load in Watts for the output:",
	...
	[title: "Touch Sensor Sensitivity Threshold", description: "This Parameter allows to adjust the Touch Button Sensitivity. " +
	...
	[title: "Brightness Control", description: "The HE-TPS01 can adjust its display brightness automatically depending on the illumination of the ambient environment and also allows to control it manually.",
	...
	[title: "Button Backlight Color", description: "This parameter defines backlight active state color",
	 name: "Selected Active State Color", options: [
				0: "Red",
				1: "Blue"
		], paramNum: 30, size: 1, default: "1", type: "enum"],

	[title: "Button Backlight Control Source", description: "This parameter defines the button backlight control source",
	 name: "Selected Source", options: [
				0: "Disabled",
				1: "Controlled by Touch Button",
				2: "Controlled by Gateway"
		], paramNum: 31, size: 1, default: "1", type: "enum"],

	[title: "Button Hold Control Mode", description: "This Parameter defines how the relay should react while holding the button. The options are: " +
	...
	[title: "Button Click Control Mode", description: "This Parameter defines how the relay should react when clicking the button. The options are: " +
	...
	[title: "Button Number for Relay Output Control", description: "This parameter defines the relay control source",
	 name: "Selected Relay Control Source", options: [
				0: "Controlled by Gateway",
				1: "Controlled by touch button",
		], paramNum: 61, size: 1, default: "1", type: "enum"],

	[title: "Relay Timer Mode Duration", description: "This parameter specify the duration in seconds for the Timer modes for Click Control Mode above. " +
	...
	"If the value is set to “0” the relay output will operate as a short contact (duration is about 0.5 sec)",
	 name: "Selected Relay Timer Mode Duration in seconds", paramNum: 71, size: 2, default: 0, type: "number", min: 0 , max: 43200, unit: "s"],

	[title: "Retore Relay State", description: "This parameter determines if the last relay state should be restored after power failure or not. " +
