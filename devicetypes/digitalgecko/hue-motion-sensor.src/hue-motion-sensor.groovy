...
metadata {
    definition (name: "Hue Motion Sensor", namespace: "digitalgecko", author: "digitalgecko", vid: "generic-motion-3", ocfDeviceType: "x.com.st.d.sensor.motion",) { //m1 no good, m2 M&B, 
...
def parse(String description) {
  ...
  Map mapz = zigbee.getEvent(description)
  def x = zigbee.parseDescriptionAsMap( description )
  log.debug "D=$description, MAP=$mapz -and- $x"
        //def x = zigbee.parseDescriptionAsMap( description )
	...
	else if (description?.startsWith('temperature: ') || description?.startsWith('illuminance: ')) {
...
//	}					  
    ...
    if (result == null) { log.warn "unparsed - $description -and- $msg "}
    log.info "P rsult end map $result -- $msg"
    //if (result != null) {createEvent(result)}
	return result
}
...
private Map getMotionResult(value) {
    //log.trace "getMotionResult : " + value
    def descriptionText = value == "01" ? "${device.displayName} detected motion": "${device.displayName} stopped detecting motion"
    //createEvent(name: 'motion', value: value == "01" ? "active" : "inactive", descriptionText: descriptionText)
    return [ name: 'motion', value: value == "01" ? "active" : "inactive", descriptionText: descriptionText, displayed:true ]
}
...
private Map getTemperatureResult(value) {
	...
	return [ name: 'temperature', value: value, descriptionText: descriptionText, unit: temperatureScale ]
}

def getTemperature(value) {
log.debug "gettemp value = $value"
	def celsius = Integer.parseInt(value, 16).shortValue() / 100
log.debug "gettemp celsius = $celsius"    
	if(getTemperatureScale() == "C"){
log.debug "gettemp matho round " + Math.round(celsius)
...
private Map getLuminanceResult(rawValue) {
    ...
    //log.trace "getLuminanceResult ${result}"
    return result
}
...
private Map parseCustomMessage(String description) {
	...
	if (description?.startsWith('temperature: ')) {
    	log.trace "${description.value} " + description.split(": ")[1]
        def value = description.split(": ")[1]
		//def value = zigbee.parseHATemperatureValue(description, "temperature: ", getTemperatureScale())
        log.trace "parseCustomMessage value $value"
    ...
    log.trace "parseCustomMessage in=$description out= $resultMap"
	return resultMap
}
...
// Unknown
    	...
    	log.warn "parseReportAttributeMessage unprocessed" + description 
    }
    //not sure what these are --- descMap.cluster == "0001" && descMap.attrId == "0021"
    
	log.trace "parseReportAttributeMessage $result"
	return result
}
