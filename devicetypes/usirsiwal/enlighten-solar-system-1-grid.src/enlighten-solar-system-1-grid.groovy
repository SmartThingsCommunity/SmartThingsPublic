...
metadata {
         definition (name: "Enlighten Solar System 1 grid", namespace: "usirsiwal", author: "Umesh Sirsiwal") {
...
def energyRefresh() {
  ...
  def cmd = "https://www.eu.solaxcloud.com:9443/proxy/api/getRealtimeInfo.do?tokenId=20210319151048132313611&sn=SWFJWDMRHU";
  log.debug "Sending request cmd[${cmd}]"

 httpGet(cmd) {resp ->
        if (resp.data) {
        	log.debug "${resp.data}"
            def energy = resp.data.result.consumeenergy
            def energyLife = resp.data.result.yieldtotal
            def currentPower = resp.data.result.feedinpower
