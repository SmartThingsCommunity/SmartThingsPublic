...
metadata {
	definition (name: "Fibaro Walli Roller Shutter Venetian", namespace: "smartthings", author: "SmartThings", ocfDeviceType: "oic.d.blind", mnmn: "SmartThings", vid: "SmartThings-smartthings-Fibaro_Roller_Shutter_Venetian", mcdSync: true) {
		...
		case 151: //Operating mode
			switch(confgurationReport.scaledConfigurationValue) {
				case 1: // "Roller blind (with positioning)"
					log.info "Changing device type to Fibaro Walli Roller Shutter"
					setDeviceType("Fibaro Walli Roller Shutter")
					break
				case 2: // "Venetian blind (with positioning)"
					log.info "Device is already configured as Fibaro Roller Shutter Venetian"
					break
...
def refresh() {
	sendHubCommand([
			encap(zwave.switchMultilevelV3.switchMultilevelGet(), 1),
			"delay 500",
			encap(zwave.switchMultilevelV3.switchMultilevelGet(), 2)
	])
}
...
def configure() {
	encap(zwave.configurationV1.configurationGet(parameterNumber: 152))
}
...
def zwaveEvent(physicalgraph.zwave.commands.switchmultilevelv3.SwitchMultilevelReport  cmd, ep = null) {
	if (cmd.value != 0xFE) {
		if (ep != 2) {
			...
			child?.sendEvent(name: "level", value: cmd.value != 0x63 ? cmd.value : 100)
		}
	} else {
...
private addVenetianBlind() {
	if (!childDevices) {
		try {
			String childDni = "${device.deviceNetworkId}:1"
			def componentLabel = "Fibaro Roller Shutter Venetian Blind"
			def child = addChildDevice("Child Venetian Blind", childDni, device.getHub().getId(), [
				completedSetup: true,
				label         : componentLabel,
				isComponent   : true,
				componentName : "venetianBlind",
				componentLabel: "Venetian Blind"
			])
		} catch(Exception e) {
...
private getParameterMap() {[
		...
		[
				...
				description: "The setting determines slats positioning in various situations."
		],
		[
