...
def zwaveEvent(physicalgraph.zwave.commands.batteryv1.BatteryReport cmd) {
	...
	def desc = "Curent battery level"
	def isStateChange = false

	if (value == 0xFF) {  // Special value for low battery alert
		value = 1
		desc = "${device.displayName} has a low battery"
		isStateChange = true
	}
	sendEvent(name: "battery", value: value, descriptionText: desc, isStateChange: isStateChange)
