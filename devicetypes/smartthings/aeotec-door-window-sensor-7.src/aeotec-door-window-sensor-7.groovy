 ...
 *
 *  Version 1.1
 *  * Parameter settings sent upon wakeup of sensor
 *  * Parameter 2 details changed (normal state vs reverse state)
 *  Version 1.0
 *  * Added Tilt Sensor to SmartThings Classic interface (does not work with connect)
...
def zwaveEvent(physicalgraph.zwave.commands.wakeupv1.WakeUpNotification cmd) {
	...
	request << update_needed_settings()
	request << "delay 1000"
...
def update_needed_settings()
{
    ...
    {   
    	if (convertParam(it.@index, settings."${it.@index}"))
        {
        	...
        	log.debug ("value is null, skipping parameter ${it.@index} setting.")
        }
...
'''
    <configuration>
      ...
      <Value type="list" index="2" label="Sensor State" min="0" max="1" value="0" byteSize="1" setting_type="zwave" displayDuringSetup="true">
            <Help>
            Reverse the state of DWS7 status
            </Help>
            <Item label="Normal state" value="0" />
            <Item label="Reverse state" value="1" />
      </Value>
    </configuration>
'''
}
