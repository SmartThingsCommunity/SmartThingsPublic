...
def zwaveEvent(physicalgraph.zwave.commands.batteryv1.BatteryReport cmd, sourceEndPoint = null) {
	def value = cmd.batteryLevel == 255 ? 0 : cmd.batteryLevel
	...
	switch(sourceEndPoint) {
		case 1:
			...
			break
		case 2:
			if(cmd.batteryLevel > 0 && !childDevices) {
				addChild()
			}
...
def zwaveEvent(physicalgraph.zwave.commands.configurationv1.ConfigurationReport cmd) {
	if(cmd.parameterNumber == 3 && cmd.scaledConfigurationValue == 1 && !childDevices) {
		addChild()
	}
}
...
private fahrenheitToCelsius(temp) {
	(temp - 32) * 5/9
}