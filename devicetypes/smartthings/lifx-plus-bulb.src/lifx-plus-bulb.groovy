...
def setColor(Map color) {
	...
	color.each { key, value ->
		switch (key) {
			case "hue":
				attrs << "hue:${(value * 3.6) as int}"
				events << createEvent(name: "hue", value: value as int)
				break
			case "saturation":
				attrs << "saturation:${value / 100}"
				events << createEvent(name: "saturation", value: value as int)
				break
...
def refresh() {
	...
	sendEvent(name: "hue", value: (data.color.hue / 3.6) as int)
	sendEvent(name: "saturation", value: (data.color.saturation * 100) as int)
