...
metadata {
	definition(name: "SmartSense Temp/Humidity Sensor", namespace: "smartthings", author: "SmartThings", runLocally: true, minHubCoreVersion: '000.017.0012', executeCommandsLocally: false, ocfDeviceType: "oic.d.thermostat") {
		...
		fingerprint profileId: "0104", deviceId: "0302", inClusters: "0000,0001,0003,0020,0402,0405", outClusters: "0003,000A,0019", manufacturer: "frient A/S", model :"HMSZB-110", deviceJoinName: "frient Multipurpose Sensor" // Develco Humidity Sensor
	}
...
def parse(String description) {
	...
	if (!map) {
		...
		map.descriptionText = temperatureScale == 'C' ? '{{ device.displayName }} was {{ value }}Â°C' : '{{ device.displayName }} was {{ value }}°F'
