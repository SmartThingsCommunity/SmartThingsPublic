...
def configure() {
	...
	if (zwaveInfo.mfr == "0086") {	// Aeon Labs meter
		...
		if (zwaveInfo.model == "004E") { //Aeotec Heavy Duty Smart Switch
			result << response(encap(zwave.configurationV1.configurationSet(parameterNumber: 90, size: 1, scaledConfigurationValue: 1))) // enabling automatic reports...
			result << response(encap(zwave.configurationV1.configurationSet(parameterNumber: 91, size: 2, scaledConfigurationValue: 10))) // ...every 10W change
		}
