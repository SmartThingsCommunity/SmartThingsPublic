 ...
 *
 * lgk I keep getting false .. xxx has arrived. There is no reason for this ... driver should keep track of past state
 * and only signal arrived if they previously have left.
 * add past state to handle this.
 * also updated function to resync past with current state.
 *
 */
metadata {
	definition (name: "LGK Arrival Sensor", namespace: "smartthings", author: "SmartThings") {
		capability "Tone"
		capability "Actuator"
...
private Map parsePresenceMessage(String description) {
   ...
   log.debug "In parse presence message!"
   // lgk dont reset state if it last state was the same
   // for some reason device is signaling and event same as last event.. ie maybe it is loosing signal
   // but dont flag this so that we done have.. xx is home then two hours later again xxx is home signaling action -- lgk
    ...
    if (name == "presence")
      {
       ...
       {
      		log.info "Found last state = $state.lastState... same new state = $value ... Ignoring!"
     		results = null
		}
        else {
             log.info "State Change valid resetting last state = $state.lastState!"
...
def updated()
{
 ...
 def currentState = device.currentValue("presence")
 state.lastState = currentState
 log.info "Set last state = $state.lastState!"
}
