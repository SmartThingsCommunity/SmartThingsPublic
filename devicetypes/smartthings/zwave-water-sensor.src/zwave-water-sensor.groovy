...
def configure() {
    if (zwaveInfo.mfr.equals("0086")) {
        def commands = []
        commands << zwave.configurationV1.configurationSet(parameterNumber: 0x5E, scaledConfigurationValue: 1, size: 1).format()
        commands << zwave.notificationV3.notificationGet(notificationType: 5).format()
        commands << zwave.batteryV1.batteryGet().format()
        response(delayBetween(commands, 1000) + ["delay 20000", zwave.wakeUpV1.wakeUpNoMoreInformation().format()])
    }
}
