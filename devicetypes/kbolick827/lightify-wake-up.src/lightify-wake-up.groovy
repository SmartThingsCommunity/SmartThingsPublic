 ...
 */
definition(
	name: "Lightify Wake Up",
	namespace: "kbolick827",
	author: "kbolick827",
...
def installed() {
	log.debug "Installing 'Lightify Wake Up' with settings: ${settings}"

	initialize()
}

def updated() {
	log.debug "Updating 'Lightify Wake Up' with settings: ${settings}"
	unschedule()
...
private initialize() {
	...
	if (!getAllChildDevices()) {
		...
		addChildDevice("kbolick827", "Lightify Wake Up Controller", dni, null, ["label": app.label])
