...
metadata {
	definition (name: "WEBipoi", namespace: "Raspberry Pi", author: "Michael Leal") {
		capability "Polling"
		capability "Refresh"
	...
	tiles {
        ...
        standardTile("restart", "device.restart", inactiveLabel: false, decoration: "flat") {
        	state "default", action:"restart", label: "Restart PI", displayName: "Restart"
        }
        
        standardTile("relay", "device.restart", inactiveLabel: false, decoration: "flat") {
        	state "default", action:"relay", label: "Toggle Storage", displayName: "Toggle Storage"
        }
        
        standardTile("storage", "device.stg", width: 1, height: 1, canChangeIcon: true) {
			state "off", action:"stg_on", label: 'Off', icon: "st.Electronics.electronics18", backgroundColor: "#ffffff", nextState: "on"
			state "on", action:"stg_off", label: 'On', icon: "st.Electronics.electronics18", backgroundColor: "#79b821", nextState: "off"        
        }
		valueTile("ST_temperature", "device.ST_temperature", width: 1, height: 1) {
            state "ST_temperature", label:'${currentValue}°', unit: "F",
            backgroundColors:[
                [value: 25, color: "#153591"],
                [value: 35, color: "#1e9cbb"],
                [value: 47, color: "#90d2a7"],
        ...
        main "button"
        details(["button", "temperature", "cpuPercentage", "memory" , "diskUsage", "restart", "relay", "ST_temperature", "storage", "refresh"])
    }
}
...
def parse(String description) {
    ...
    if (result.containsKey("stg_temp")) {
    	log.debug "stg_temp: ${result.stg_temp}"
    	sendEvent(name: "ST_temperature", value: result.stg_temp)
    }
    
    if (result.containsKey("pwr_status")) {
    	log.debug "pwr_status: ${result.pwr_status}"
    	if(result.pwr_status){
        	sendEvent(name: "stg", value: "off")
            log.debug "Setting power status to off"
		}
        else{
        	sendEvent(name: "stg", value: "on")
            log.debug "Setting power status to on"
        }
	}
}
...
def stg_restart(){
	log.debug "Restart storage was pressed"
    sendEvent(name: "switch", value: "off")
    getRPiData()
}

def stg_on(){
	log.debug "Turning on storage"
	def uri = "/macros/stg_on"
    postAction(uri)
}


def stg_off(){
	log.debug "Turning off storage"
	def uri = "/macros/stg_off"
    postAction(uri)
}

def relay(){
	log.debug "On/Off relay"
	def uri = "/macros/relay"
    postAction(uri)
}
