/*
MCOHome Touch Panel MH-S314-EU Description

zw:L - Listening
type:1001
mfr:015F (Device manufacturer)
prod:3141 (Product Type ID)
model:5102 (Product ID)
ver:5.03 (Application firmware version)
...
metadata {
	definition (name: "MCOHome Touch Panel MH-S314-EU", namespace: "automateasia", author: "edwin") {
		capability "Switch"
		...
		command "childOn"
		command "childOff"
		fingerprint mfr: "015F", prod: "3141", model: "5102"
	}
	...
	tiles(scale: 2) {
		...
		childDeviceTile("channel-3", "channel-3", height: 3, width: 3, childTileName: "switch")
		childDeviceTile("channel-4", "channel-4", height: 3, width: 3, childTileName: "switch")
	}

	main("status")
	details(["channel-1", "channel-2", "channel-3","channel-4", "refresh"])
}
...
def childOn(String dni) {
	...
	} else if (dni=="${device.deviceNetworkId}-3") {
    		delayBetween([
	...
	} else if (dni=="${device.deviceNetworkId}-4") {
    		delayBetween([
