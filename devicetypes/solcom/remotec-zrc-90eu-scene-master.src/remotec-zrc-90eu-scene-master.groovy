// Adapted from Remotec ZRC-90 Scene Master by Eric Maycock to return number of button pushed in "numberofButtons"
metadata {
    definition (name: "Remotec ZRC-90EU Scene Master", namespace: "solcom", author: "Mark Dunsford") {
		capability "Actuator"
		capability "Button"
	...
	tiles (scale: 2) {
		multiAttributeTile(name:"button", type:"generic", width:6, height:4) {
  			tileAttribute("device.numberOfButtons", key: "PRIMARY_CONTROL"){
    		attributeState "default", label:'${currentValue}', backgroundColor:"#ffffff", icon: "st.unknown.zwave.remote-controller"
  			}
            tileAttribute ("device.button", key: "SECONDARY_CONTROL") {
			attributeState "state", label:'${currentValue}'
            }
            
        }
        valueTile(
			"number", "device.numberOfButtons", decoration: "flat", width: 2, height: 2) {
			state "number", label:'${currentValue}', unit:""
		}
        valueTile(
        ...
        valueTile(
			...
			state "sequenceNumber", label:'${currentValue}', unit:""
		}
        standardTile("configure", "device.configure", inactiveLabel: false, width: 2, height: 2, decoration: "flat") {
			state "configure", label:'', action:"configuration.configure", icon:"st.secondary.configure"
		}
		main "button"
		details(["button", "number", "battery", "sequenceNumber", "configure"])
	}
...
def results = [createEvent(name: "button", value: value, data: [buttonNumber: button], descriptionText: "$device.displayName button $button was $value", isStateChange: true)]
	results << createEvent(name: "numberOfButtons", value: button, displayed: false)
    return results
}
