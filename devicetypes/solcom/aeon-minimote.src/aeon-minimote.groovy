...
/**
 *  Adapted from Aeon Minimote Device Handler - Copyright 2015 SmartThings
 *
 ...
 */
metadata {
	definition (name: "Aeon Minimote", namespace: "solcom", author: "Mark Dunsford", minHubCoreVersion: '000.017.0012') {
		capability "Actuator"
		capability "Button"
...
def buttonEvent(button, held) {
	...
	if (held) {
		...
		def result = [createEvent(name: "button", value: "held", data: [buttonNumber: button], descriptionText: "$device.displayName button $button was held", isStateChange: true)]
        result << createEvent(name: "numberOfButtons", value: button, isStateChange: true)
        return result
	} else {
