...
def on() {
    ...
    sendData("${endereco}/acionar/2", onHandler )
}
...
def off() {
  def endereco = device.currentValue("endereco")
  sendData("${endereco}/acionar/3", offHandler )
}

def pause() {
  def endereco = device.currentValue("endereco")
  sendData("${endereco}/acionar/1", pauseHandler )
}
