/**
  ...
  */
  ...
  def selectPhrases() {
    ...
    dynamicPage(name: "selectPhrases", title: "Configure", nextPage:"Settings", uninstall: true) {
      ...
      if (phrases) {
        ...
        section("Select modes used for each condition.") {
        ...
        section("And offset sunset by how many minutes (optional)?") {
          input "sunsetOff", "number", title: "How long?", description: "Negative numbers will be before sunset.", multiple: false, required: false
        }

        section("And offset sunrise by how many minutes (optional)?") {
          input "sunriseOff", "number", title: "How long?", description: "Negative numbers will be before sunrise.", multiple: false, required: false
        }
      }
    }
  }
  ...
  def initialize() {
    state.delay = (falseAlarmThreshold != null && falseAlarmThreshold != "") ? falseAlarmThreshold * 60 : 10 * 60
  ...
  // convert integer minutes into an "hh:mm" string
  private minutesToOffset(offset) {
    if (offset == null || offset == "" || offset > 1440 || offset < -1440) {
      return "00:00"
    }
    def sign = offset < 0 ? "-" : ""
