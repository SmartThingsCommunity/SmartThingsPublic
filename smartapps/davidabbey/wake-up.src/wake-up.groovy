...
def rootPage() {
	dynamicPage(name: "rootPage", title: "dabbeyRoot", install: true, uninstall: true) {
		...
		if (dimmers) {
			...
			section("Wake Up!! Has A Controller") {
				href(title: "Learn how to control Wake Up!!", page: "controllerExplanationPage", description: null)
			}
...
def controllerExplanationPage() {
	dynamicPage(name: "controllerExplanationPage", title: "How To Control Wake Up!!") {
		...
		section("With other SmartApps", hideable: true, hidden: false) {
			...
			paragraph "The controller acts like a switch so any SmartApp that can control a switch can control Wake Up!!, too!"
			paragraph "Routines and 'Smart Lighting' are great ways to automate Wake Up!!"
		}
		...
		section("More about the controller", hideable: true, hidden: true) {
			...
			paragraph "You can start and stop Wake Up!! by tapping the control on the right."
			image "http://f.cl.ly/items/3W323J3M1b3K0k0V3X3a/GentleWakeUpController-tile-running.png"
			paragraph "If you look at the device details screen, you will find even more information about Wake Up!! and more fine grain controls."
			image "http://f.cl.ly/items/291s3z2I2Q0r2q0x171H/GentleWakeUpController-richTile-stopped.png"
			paragraph "The slider allows you to jump to any point in the dimming process. Think of it as a percentage. If Wake Up!! is set to dim down as you fall asleep, but your book is just too good to put down; simply drag the slider to the left and Wake Up!! will give you more time to finish your chapter and drift off to sleep."
...
def installed() {
	log.debug "Installing 'GWake Up!!' with settings: ${settings}"

	initialize()
}

def updated() {
	log.debug "Updating 'Wake Up!!' with settings: ${settings}"
	unschedule()
...
private initialize() {
	...
	if (!getAllChildDevices()) {
		...
		addChildDevice("smartthings", "Wake Up!!", dni, null, ["label": app.label])
