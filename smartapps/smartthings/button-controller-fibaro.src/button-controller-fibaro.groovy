 ...
 */
definition(
    name: "Button Controller Fibaro",
    namespace: "smartthings",
    author: "SmartThings",
    description: "Control devices with Fibaro Button",
...
def configureButton4() {
	dynamicPage(name: "configureButton4", title: "If you have a fourth button, you can do even more here",
...
def configureButton6() {
	dynamicPage(name: "configureButton6", title: "If you have a starting held",
		nextPage: "configureButton7", uninstall: configured(), getButtonSections(6))
}

def configureButton7() {
	dynamicPage(name: "configureButton7", title: "If you have a ending held",
...
def getButtonSections(buttonNumber) {
	return {
		section("Lights") {
			if(buttonNumber <= 5) input "lights_${buttonNumber}_pushed", "capability.switch", title: "Pushed", multiple: true, required: false
			if(buttonNumber >= 6) input "lights_${buttonNumber}_held", "capability.switch", title: "Held", multiple: true, required: false
		}
		section("Locks") {
			if(buttonNumber <= 5) input "locks_${buttonNumber}_pushed", "capability.lock", title: "Pushed", multiple: true, required: false
			if(buttonNumber >= 6) input "locks_${buttonNumber}_held", "capability.lock", title: "Held", multiple: true, required: false
		}
		section("Sonos") {
			if(buttonNumber <= 5) input "sonos_${buttonNumber}_pushed", "capability.musicPlayer", title: "Pushed", multiple: true, required: false
			if(buttonNumber >= 6) input "sonos_${buttonNumber}_held", "capability.musicPlayer", title: "Held", multiple: true, required: false
		}
		section("Modes") {
			if(buttonNumber <= 5) input "mode_${buttonNumber}_pushed", "mode", title: "Pushed", required: false
			if(buttonNumber >= 6) input "mode_${buttonNumber}_held", "mode", title: "Held", required: false
		}
		def phrases = location.helloHome?.getPhrases()*.label
		if (phrases) {
			section("Hello Home Actions") {
				log.trace phrases
				if(buttonNumber <= 5) input "phrase_${buttonNumber}_pushed", "enum", title: "Pushed", required: false, options: phrases
				if(buttonNumber >= 6) input "phrase_${buttonNumber}_held", "enum", title: "Held", required: false, options: phrases
			}
		}
        section("Sirens") {
            if(buttonNumber <= 5) input "sirens_${buttonNumber}_pushed","capability.alarm" ,title: "Pushed", multiple: true, required: false
            if(buttonNumber >= 6) input "sirens_${buttonNumber}_held", "capability.alarm", title: "Held", multiple: true, required: false
        }
        ...
        section("Push Notifications") {
            if(buttonNumber <= 5) input "notifications_${buttonNumber}_pushed","bool" ,title: "Pushed", required: false, defaultValue: false
            if(buttonNumber >= 6) input "notifications_${buttonNumber}_held", "bool", title: "Held", required: false, defaultValue: false
        }

        section("Sms Notifications") {
            if(buttonNumber <= 5) input "phone_${buttonNumber}_pushed","phone" ,title: "Pushed", required: false
            if(buttonNumber >= 6) input "phone_${buttonNumber}_held", "phone", title: "Held", required: false
        }
	}
}
