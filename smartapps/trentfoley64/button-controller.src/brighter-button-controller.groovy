 ...
 *	Author: SmartThings
 *	Date: 2014-5-21
 *  Modified By: trentfoley64
 *  Date: 2015-11-04
 *
 *  Modified to add dimming level for lights
 */
definition(
    name: "Brighter Button Controller",
...
def getButtonSections(buttonNumber) {
	return {
      ...
      input "brightness_${buttonNumber}_pushed", "number", range: "1-100", title: "Pushed", required: false
			input "lights_${buttonNumber}_held", "capability.switch", title: "Held", multiple: true, required: false
      input "brightness_${buttonNumber}_held", "number", range: "1-100", title: "Held", required: false
		}
		section("Locks") {
...
def executeHandlers(buttonNumber, value) {
  ...
  def brightness = find('brightness', buttonNumber, value)
	if (lights != null) toggle(lights,brightness)
...
def toggle(devices,brightness) {
	log.debug "toggle: $devices = ${devices*.currentValue('switch')}, brightness: $brightness, switchLevel: ${devices*.currentValue('switchLevel')}"

	if (devices*.currentValue('switch').contains('on') || devices*.currentValue('switchLevel')) {
		devices.off()
	}
	else if (devices*.currentValue('switch').contains('off')) {
    brightness ? devices.setLevel(brightness) : devices.on()
	}
}
