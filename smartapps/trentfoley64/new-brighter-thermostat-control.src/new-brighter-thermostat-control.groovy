...
def setName() {
	dynamicPage(name: "setName", title: "Brighter Thermostat Control", install: true, uninstall: true) {
		section("Control name") {
			label title: "Assign a name", required: true, description: getDefaultLabel(), default: "$thermostats"
...
private getDefaultLabel() {
	...
	def msg="$thermostats $time $daysOfWeekList $anyMustBePresent $allMustBepresent $anyMustBeAbsent $allMustBeAbsent"
	return msg
}
...
private nextDayOfWeekDate(startTime,daysOfWeekList) {
    ...
    for(def i=0; i<6; i++) {
        if (daysOfWeekList.contains(nextScheduleTime.format("EEEE",location.timeZone))) {
        	// found the dayOfWeek so return it
        	return nextScheduleTime
        }
        // nextScheduleTime wasn't in our daysOfWeekList so skip to next day
        nextScheduleTime=nextScheduleTime + 1
    }
    // If we get to this point, we are on the wrong planet and something is very wrong.
	log.debug "Wrong Planet Error. Unable to compute nextDayOfWeekDate for $startTime and $daysOfWeekList."
	return null
}
