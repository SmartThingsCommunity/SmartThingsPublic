...
def buttonPushedHandler(evt) {
  ...
  if (buttonNumber==1) {
    log.info "Button 1 pushed (on)"
    targets.on()
    targets.setLevel(100)
  } else {
    log.info "Button 2 pushed (off)"
    targets.off()
  }
}
...
def buttonHeldHandler(evt) {
  ...
  if (buttonNumber==1) {
    log.info "Button 1 held (Setting brightness to $upLevel)"
    targets.setLevel(upLevel)
  } else {
    log.info "Button 2 held (Setting brightness to $downLevel)"
