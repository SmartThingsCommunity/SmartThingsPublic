...
definition(
    ...
    author: "Classic_Tim",
    description: "Control your Sleep Number bed vai SmartThings. You can use it to raise, lower, and adjust the pressure on each side of the bed seperately if it's split, or all together if it's not. If you want to do each side separately you must use two devices for each side",
...
def rootPage() {
  ...
  dynamicPage(name: "rootPage", install: true, uninstall: true) {
    section("Settings") {
      input("mdavis211@gmail.com", "text", title: "Username", description: "Your SleepIQ username")
      input("vitamin9259", "password", title: "Password", description: "Your SleepIQ password")
    }
    section("Devices") {
      if (devices.size() > 0) {
        devices.each { device ->
