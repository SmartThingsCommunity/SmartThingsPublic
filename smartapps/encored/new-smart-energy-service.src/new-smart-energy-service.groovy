...
def updated() {
    ...
    if (!atomicState.notPaired) {  
       ...
       def energyPlanUsage = getHttpGetJson(changeToUsageParam)
        def epUsage = 0
        if (energyPlanUsage) {
        	log.debug "getin herhehrehrehrehrehrehrehehrehehrehherherhehrehehrehehrehrehrehrehhr"
		...
		log.debug "Usage epepepepepepepepepepepepepepepepepepeeppepepepeeppepepepepepepeep ${epUsage}"
...
def initialize() {
    ...
    if (EATValidation) {
        if (getUUID()) {
        //if (false) { // to test things without uuid.
...
private createLocaleStrings() {
   ...
   [
    	...
    	en : [
                ...
                htmlText : [
                			rankingShow : "collecting data",
                            meteringPeriodBillFalse : "collecting data",
                            lastMonthFalse : "no records",
                       
                            planFalse : "set up plan",
                            thisMonthTitle : "This Month",
                            tierTitle : "Billing Tier",
                            planTitle : "Plan", 
...
*	If the token get refreshed, it will refresh the value of Encored Access Token
...
private getDeviceStatus() {		
    ...
    def onOff = "OFF"
    def htmlText = [
    			ko : [enertalkDeviceTitle : "스마트미터 상태"],
                en : [enertalkDeviceTitle : "EnerTalk Device"]
    			]
    if (atomicState.EATValidation) {
        ...
        /* get device status. */
        statusData = getHttpGetJson(deviceStatusParam)
    ...
    atomicState.device = [ 
        ...
        status 		: status,
        html 		: """	<div class="contents tail" id="content7">
              					<p class="key" id="korean-device" >${htmlText."${atomicState.language}".enertalkDeviceTitle}</p>
...
private getStandbyPower() {
    ...
    if (atomicState.EATValidation) {
        ...
        if (standbyData) {
            if (standbyData.standbyPower) {
                standbyShow = (standbyData.standbyPower / 1000)
                htmlText."${state.language}".standbyFalse = ""
                standbyUnit = "Wh"
            }
        }
    }
    atomicState.standby = [ 
        ...
        standby 	: stanbyShow,
        html		: """ <div class="contents tail" id="content6">
                          <p class="key" id="korean-standby">${htmlText."${atomicState.language}".standbyTitle}</p>
                          <span class="value-block">
                            <p class="value" id="value-standby">${standbyShow}</p>
                            <p class="value" id="value-fail">${htmlText."${atomicState.language}".standbyFalse}</p>
...
private getUserInfo() {
	...
	if (atomicState.EATValidation) {
		...
		log.debug "asjdfhlafhlashfasjkdhfjkashdflkadshfkljashdfjkashdfjkhasdkfjhasdlkfhaskldfhalskdhfaksjdhflaskdhflkjasdhflkjadshflkhjasd"
        /* Parse max limit usage and it's bill from user's info. */
        if (meData) {
        	log.debug "getting meememememememmememememememmememememmemememememmememememe ${meData}"
            if (meData.maxLimitUsageBill) {
            	log.debug "들어와라 들어와라 들어와라 들어와라"
                maxLimitUsageBill = meData.maxLimitUsageBill
                maxLimitUsage = meData.maxLimitUsage
                log.debug meData.maxLimitUsageBill
                log.debug meData.maxLimitUsage
            }
        }
    }
    
    atomicState.userInfo = [
    						maxLimitUsageBill : maxLimitUsageBill,
                            maxLimitUsage : maxLimitUsage
    						]
}

private getMeteringValues(){
	def meteringPeriodBill = ""
    def plan = ""
    def start = ""
    def end = ""
    def meteringDay = ""
	...
	if (atomicState.EATValidation) {
        ...
        def meteringData = getHttpGetJson(meteringParam)
        ...
        */	
        if (meteringData) {
            if (meteringData.meteringPeriodBill) {
                ...
                plan = atomicState.userInfo.maxLimitUsageBill - meteringData.meteringPeriodBill
...
private getRanking() {
	def rankingData = ""
    def rankImage = ""
    def htmlText = [ ko : [rankingShow : "데이터 수집 중",
    					   rankingTitle : "랭킹"],
    				 en : [rankingShow : "collecting data",
                           rankingTitle : "Ranking"]]
    def percent = ""
	...
	if (atomicState.EATValidation) {
        ...
        if (rankingData) {
            if (rankingData.user.ranking) {
                ...
                htmlText."${atomicState.language}".rankingShow = ""
            }
        }
    }
    ...
    atomicState.ranking = [
                            ...
                            percent : percent,
                            html : """ <div class="contents tail" id="content5">
                                          <p class="key" id="korean-ranking">${htmlText."${atomicState.language}".rankingTitle}</p>
                                          <span class="value-block">
                                              <div id="value-block-rank">${rankImage}</div>
                                              <p class="value" id="value-fail">${htmlText."${atomicState.language}".rankingShow}</p>
                                          </span>
									  </div> """
    					  ]
}
...
private getLastMonth() {
    ...
    def lastMonth = ""
    def htmlText = [
    				ko : [lastMonthTitle : "지난달",
                    	  lastMonthFalse : "정보가 없습니다" ],
                    en : [lastMonthTitle : "Last Month",
                    	  lastMonthFalse : "no records"]
    				]
    if (atomicState.EATValidation) {
        /* if the start value exist, get last month energy usage. */
        if (atomicState.meterValues.start) {
            def lastMonthParam = makeGetParams( "http://api-staging.encoredtech.com/1.2/devices/${atomicState.uuid}/meteringUsages?period=monthly&start=${atomicState.meterValues.start}&end=${atomicState.meterValues.end}",
                                               [Authorization: "Bearer ${atomicState.encoredAccessToken}", ContentType: "application/json"])
            ...
            if (lastMonth) {
                ...
                htmlText."${atomicState.language}".lastMonthFalse = ""
                lastMonthUnit = "&#x20A9;"
            }

        }
	}
    atomicState.lastMonth = [
    						lastMonth : lastMonth,
    						html : """ <div class="contents tail" id="content4">
                                        	<p class="key" id="korean-last">${htmlText."${atomicState.language}".lastMonthTitle}</p>
                                        	<span class="value-block">
                                        		<p class="unit first" id="unit-first-last">${lastMonthUnit}</p>
                                        		<p class="value" id="value-last">${lastMonthShow}</p>
                                        		<p class="value" id="value-fail">${htmlText."${atomicState.language}".lastMonthFalse}</p>
                                        	</span>
                                        </div>"""
            				]
}
...
private getPlan() {
	...
	if (atomicState.EATValidation) {
    	def plan = atomicState.meterValues.plan
        if (plan) {
            planShow = plan
            if (plan >= 1000) {
            	planShow = formatMoney("${plan}") 
            }
            htmlText."${atomicState.language}".planFalse = ""
            planUnitOne = "&#x20A9;"

        }
	}
    atomicState.plan = [ html : """ <div class="contents tail" id="content3">
                                      <p class="key" id="korean-plan">${htmlText."${atomicState.language}".planTitle}</p>
                                      <span class="value-block">
                                        <p class="unit first" id="unit-first-plan">${planUnitOne}</p>
                                        <p class="value" id="value-plan">${planShow}</p>
                                        <p class="value" id="value-fail">${htmlText."${atomicState.language}".planFalse}</p>
                                        <p class="unit second" id="unit-second-plan"> ${htmlText."${atomicState.language}".planUnitTwo}</p> 
                                      </span>
                                    </div> """]
}
...
private getTier() {
	...
	atomicState.tier = [html : """<div class="contents tail" id="content2">
                                  <p class="key" id="korean-tier">${htmlText."${atomicState.language}".tierTitle}</p>
                                  <span class="value-block">
                                    <div id="value-block-tier"><img id=\"image-tier\" src=\"https://s3-ap-northeast-1.amazonaws.com/smartthings-images/tier_${atomicState.meterValues.tier}.svg\" /></div>
                                    <p class="value" id="value-tier">${atomicState.meterValues.tier}</p>
                                  </span>
                                </div>"""]
}
...
private getThisMonth() {
    ...
    def meteringPeriodBillShow = ""
    def thisMonthUnitOne = ""
    def thisMonthUnitTwo = ""
    def htmlText = [
    				ko : [thisMonthTitle : "이번달",
                    	  meteringPeriodBillFalse : "데이터 수집 중"],
                    en : [thisMonthTitle : "This Month",
                    	  meteringPeriodBillFalse : "collecting data"]
    				]
    def meteringPeriodBill = atomicState.meterValues.meteringPeriodBill
    ...
    if (meteringPeriodBill) {
        ...
        htmlText."${atomicState.language}".meteringPeriodBillFalse = ""
        thisMonthUnitOne = "&#x20A9;"

        def dayPassed = getDayPassed(atomicState.meterValues.start, atomicState.meterValues.end, atomicState.meterValues.meteringDay)
	...
	atomicState.thisMonth = [html : """	<div class="contents head" id="content1">
                                          <p class="key" id="korean-this">${htmlText."${atomicState.language}".thisMonthTitle}</p>
                                          <span class="value-block">
                                            <p class="unit first" id="unit-first-this">${thisMonthUnitOne}</p>
                                            <p class="value" id="value-this">${meteringPeriodBillShow}</p>
                                            <p class="value" id="value-fail">${htmlText."${atomicState.language}".meteringPeriodBillFalse}</p>
...
def getHtml() {
    ...
    lastMonthTitle = "Last Month", standbyTitle = "Standby", energyMonitorDeviceTitle = "Energy Monitor Device"
   ...
   /* check Encored Access Token */
    atomicState.EATValidation = checkEncoreAccessTokenValidation()
    
    /* check if uuid already exist or not.*/
    if (atomicState.EATValidation && atomicState.notPaired) {
    	getUUID()
    }
    ...
    if (!atomicState.notPaired) {
        /* Device Status */
        getDeviceStatus()
        log.debug atomicState.device
        
        /* standby power.*/
        getStandbyPower();
        log.debug atomicState.standby
        
        /* user info (me) */
        getUserInfo()
        log.debug atomicState.userInfo
        
       	/* metering related values. */
		getMeteringValues()
        log.debug atomicState.meterValues
        
        /* ranking */
		getRanking()
        log.debug atomicState.ranking
        ...
        /* last month */       
		getLastMonth()
        log.debug atomicState.lastMonth
                
        /* plan */
		getPlan()
        log.debug atomicState.plan
             
		/* tier */
        getTier()
        log.debug atomicState.tier
        
        /* this month */
        getThisMonth()
        log.debug atomicState.thisMonth
       ...
       /* I decided to set values to device type handler, on loading solution module. 
        ...
        def d = getChildDevice(atomicState.dni)
        def kWhMonth = Math.round(atomicState.meterValues.meteringUsage / 10000) / 100 /* milliwatt to kilowatt*/
        def planUsed = 0
        if ( maxLimitUsage > 0 ) {
        	planUsed = Math.round((atomicState.meterValues.meteringUsage / atomicState.userInfo.maxLimitUsage) * 100) /* get the pecent of used amount against max usage */
        } else {
        	planUsed = Math.round((atomicState.meterValues.meteringUsage/ 1000000) * 100) /* if max was not decided let the used value be percent. e.g. 1kWh = 100% */
        }
        ...
        d?.sendEvent(name: "tier", value : "${atomicState.meterValues.tier}")
        d?.sendEvent(name: "plan", value :"${planUsed}")
        deviceId = d.id
        
    } else {
        
        log.debug "UUID does not exist."
    }
    ...
    atomicState.solutionModuleSettings = [
       ...
       deviceState    : atomicState.device.status, 
       percent        : atomicState.ranking.percent,
	...
	renderHTML() {
        ...
        """
         <div id="real-time">
            ...
            <!-- this month section -->
            ${atomicState.thisMonth.html}
            
            <!-- Billing Tier section -->
			${atomicState.tier.html}
            
            <!-- Plan section -->
            ${atomicState.plan.html}
            
            <!-- Last Month section -->
            ${atomicState.lastMonth.html}
            
            <!-- Ranking section -->
            ${atomicState.ranking.html}
            
            <!-- Standby section -->
            ${atomicState.standby.html}
            
            <!-- Device status section -->
            ${atomicState.device.html}
          </div>
